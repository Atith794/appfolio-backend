...
import { BILLING_CATALOG, } from "../lib/billingConfig.js";
...
function isBillingRegion(value) {
...
function hasLockedBillingProfile(user) {
    return Boolean(user?.billingCountry &&
        user?.billingCurrency &&
        user?.billingRegion &&
        user?.provider);
}
function resolvePricingForUser(req, user) {
    if (hasLockedBillingProfile(user) && isBillingRegion(user.billingRegion)) {
        const region = user.billingRegion;
...
function hasOpenSubscriptionState(user) {
    const activeLike = ["ACTIVE", "TRIALING", "PAST_DUE"];
    if (user?.plan === "PRO" && activeLike.includes(String(user?.planStatus))) {
        return true;
    }
    const hasProviderSubscription = Boolean(user?.razorpaySubscriptionId) ||
...
function setPlanFromStripeStatus(user, status) {
...
function setPlanFromRazorpayStatus(user, status) {
    const s = String(status || "").toLowerCase();
    if (s === "active") {
        user.plan = "PRO";
        user.planStatus = "ACTIVE";
...
function isStillWithinPaidPeriod(user) {
    if (!user?.planValidUntil)
        return false;
    return new Date(user.planValidUntil).getTime() > Date.now();
}
function getRazorpayTotalCount(billing) {
    if (billing === "MONTHLY") {
        return 100;
    }
    if (billing === "YEARLY") {
...
export async function billingRoutes(app) {
    ...
    app.post("/razorpay/verify-subscription", {
        ...
        const expected = hmacSHA256Hex(`${razorpay_payment_id}|${subId}`, process.env.RAZORPAY_KEY_SECRET);
        ...
        else if (subscriptionStatus === "authenticated" ||
