...
function isBilling(value) {
    return value === "MONTHLY" || value === "YEARLY";
}
function isBillingCurrency(value) {
    return (value === "INR" ||
        value === "USD" ||
        value === "GBP" ||
        value === "EUR");
}
function addBillingPeriod(start, billing) {
...
export async function paymentsRoutes(app) {
    ...
    app.post("/verify-payment", {
        ...
        const expected = hmacSHA256Hex(`${razorpay_order_id}|${razorpay_payment_id}`, process.env.RAZORPAY_KEY_SECRET);
        if (!safeEqualHex(expected, razorpay_signature)) {
            return reply.code(400).send({ message: "Invalid payment signature" });
        }
        const order = await razorpay.orders.fetch(razorpay_order_id);
        const orderNotes = order?.notes || {};
        if (orderNotes.clerkUserId &&
